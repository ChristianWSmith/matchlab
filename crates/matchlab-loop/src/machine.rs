...
pub struct LoopConfig {
    ...
    /// Per-stream seeds: each stochastic subsystem draws from its own RNG, so
    /// experiments can hold one stream fixed (common random numbers) while
    /// varying another (spec §13.7).
    pub stream_seeds: StreamSeeds,
    /// Record every match + participant snapshot into `MachineState.history`
    /// (the recording arm of a counterfactual study, spec §13.8,).
...
pub fn handle_player_queue(
    ...
    if let Some(obs) = world.observe(pid).cloned() {
        ...
        tracing::debug!(player_id = pid.0, "player enqueued");
...
pub fn handle_match_timer(
    ...
    if remaining > 0 {
        ...
        let now = world.time;
        let proposed = state.matchmaker.find_matches(
            &state.queue,
            world,
            &teams,
            now,
...
pub fn handle_match_formed(
    ...
    let result = state
        .outcome_model
        .simulate(match_id, &team_a, &team_b, &mut state.game_rng);
