...
/// Build a `MatchLoop` for the standard elo + logistic + matchmaker stack with
/// an explicit `TeamComposition` (XvY sizes + optional roles).
pub fn build_loop(
...
/// Like `build_loop` but with configurable outcome + matchmaker scripts and
/// YAML params (used by the metamorphic suite to perturb game noise and widen
/// the matchmaking window).
#[allow(clippy::too_many_arguments)]
pub fn build_loop_full(
    population: Vec<(PlayerReality, PlayerObservation)>,
    teams: TeamComposition,
    max_matches: u64,
    seed: u64,
    ...
    let matchmaker = LuaMatchmaker::load(
        matchmaker_script,
        &serde_yaml::from_str(matchmaker_params).expect("valid matchmaker params"),
    )
...
/// Like `run_loop` but with configurable game + matchmaker scripts.
#[allow(clippy::too_many_arguments)]
pub fn run_loop_full(
    population: Vec<(PlayerReality, PlayerObservation)>,
    teams: TeamComposition,
    max_matches: u64,
    max_time_secs: f64,
