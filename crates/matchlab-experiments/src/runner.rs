...
impl ExperimentRunner {
    /// Run an experiment to completion and return its `ExperimentResult`.
    pub fn run(config: &ExperimentConfig) -> Result<ExperimentResult, String> {
        Ok(Self::run_recording(config, false)?.0)
    }
    /// Run an experiment, optionally capturing the counterfactual recording
    /// trace (spec §13.8,). When `record_history` is set the matched
    /// `GameHistory` is returned alongside the `ExperimentResult`; the trace
    /// is otherwise unused and the run is byte-identical to `run`.
    pub fn run_recording(
        config: &ExperimentConfig,
        record_history: bool,
    ) -> Result<(ExperimentResult, Option<matchlab_loop::GameHistory>), String> {
        ...
        tracing::info!(script = %config.experiment.game.script, "outcome model loaded");
        let matchmaker = build_matchmaker(&config.experiment.matchmaking)?;
        tracing::info!(script = %config.experiment.matchmaking.script, "matchmaker loaded");
        ...
        let loop_config = LoopConfig {
            ...
            stream_seeds: StreamSeeds::from_seed(seeds.experiment_seed),
            record_history,
        };
        ...
        let history = loop_.state.lock().unwrap().history.take();
        Ok((result, history))
    }
}
...
pub(crate) fn iso8601_utc() -> String {
