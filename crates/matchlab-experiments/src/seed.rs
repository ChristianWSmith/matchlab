...
//!
//! `SeedManager` derives independent seeds for population, games, arrivals,
//! behavior, matchmaking, and the master world RNG from a single experiment
//! seed, so one manifest value reproduces the whole run. `hash_config` and
//! `git_commit_hash` give every `ExperimentResult` an auditable fingerprint of
//! the exact config + code version that produced it.
...
mod tests {
    ...
    fn derives_distinct_stable_seeds() {
        ...
        assert_eq!(seeds.matchmaker_seed, derive(42, 5));
        assert_eq!(seeds.master_seed, derive(42, 6));
        let distinct = [
            seeds.population_seed,
            seeds.game_seed,
            seeds.arrival_seed,
            seeds.behavior_seed,
            seeds.matchmaker_seed,
            seeds.master_seed,
        ];
