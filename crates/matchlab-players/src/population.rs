    ...
    pub fn generate(
        ...
        for (archetype, &count) in config.archetypes.iter().zip(counts.iter()) {
            for _ in 0..count {
                ...
                id_counter += 1;
                let (skill_vector, overall_skill) = if archetype.is_multidimensional() {
                    let dims = archetype.skill_dimensions.as_ref().unwrap();
                    let mut dimensions = std::collections::HashMap::new();
                    let mut overall = 0.0;
                    let mut n_dims = 0;
                    for (dim_name, dim_spec) in dims {
                        let val = sample_distribution(&dim_spec.distribution, rng);
                        dimensions.insert(dim_name.clone(), val);
                        overall += val;
                        n_dims += 1;
                    }
                    if n_dims > 0 {
                ...
                let rating = archetype.initial_rating.unwrap_or(overall_skill);
                realities.push(PlayerReality {
                    id,
                    skill: skill_vector.clone(),
